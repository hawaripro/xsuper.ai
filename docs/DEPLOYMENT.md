...
- Modal fal per gambar/megapiksel dikalikan jumlah hasil maksimum yang dapat dipilih bila kontrak v2 menagih per permintaan (mis. `num_images` 1–4 → modal ×4); eksekusi native v1 menagih per hasil lewat jumlah dan tidak dikalikan lagi. Selama model masih berjalan native, hanya revisi v2 yang sudah terbit yang dihitung; kandidat impor atau revisi yang baru ditinjau tidak melipatgandakan harga. Jumlah maksimum yang tidak terbatas membuat modal tidak diketahui.
