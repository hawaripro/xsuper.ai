...
- Saldo tidak cukup sebelum panggilan LLM menghasilkan HTTP **402**, tanpa request provider. Pada API developer, usage final hilang/tidak valid atau shortfall tetap menahan reservasi untuk pemeriksaan; jangan mengembalikan saldo seolah-olah provider belum menghasilkan jawaban.
- **Chat web**: member hanya ditawari model dengan tarif input dan output aktif. Admisi membekukan prompt sistem, notes/lampiran, tarif, dan batas output yang terjangkau (minimal 256 token), lalu membuat reservasi `chat:{operation_uuid}`. Admin tidak memerlukan dompet dan tidak ditagih. Replay kunci permintaan yang sama tidak menagih ulang; retry/lanjutkan adalah percobaan baru.
- **Penyelesaian chat**: usage provider yang sah menjadi biaya final (termasuk tarif cache); jika usage tidak tersedia tetapi sudah ada teks, input memakai estimasi admisi dan output memakai `ceil(jumlah karakter/3)`, ditandai `billing.estimated`. Tanpa teks/usage, saldo kembali penuh. Stop, kedaluwarsa, gagal, dan penghapusan percakapan mengikuti aturan yang sama, tepat sekali. Shortfall bertanda `held` tetap didebit, bukan pendapatan final, tanpa merusak SSE. Poll/final SSE hanya mengirim status dan `cost_usd`, bukan snapshot tarif.
- **UI chat**: chip Saldo AI diperbarui setelah jawaban/402 dan menuju `/deposit?tab=wallet`; ekuivalen IDR mengikuti kurs deposit. Penolakan 402 mempertahankan draft dan tidak menyisakan bubble gagal. Harga model ditampilkan per sejuta token masuk/keluar. Pendapatan usage admin mencakup biaya chat web yang sudah settled.
...
- Video native **Kinovi dan fal ditagih per detik yang dipilih** setelah harganya diterapkan dari modal yang diketahui. Kinovi tetap menawarkan 5/8/10 detik. Fal mengalikan tarif per detik dengan durasi dan, bila dipilih, kualitas Pro ×2; tarif modal mengakomodasi konfigurasi/referensi/tier termahal per detik yang dapat dipilih. Harga sesi realtime tetap per sesi/permintaan, bukan per detik.
- Satuan tarif media tersimpan di `ai_model_profiles.token_cost_unit` (kosong = satuan bawaan integrasi native). Migrasi `2026_09_26_000100` menandai tarif positif lama LongCat fal (2 endpoint) dan enam video Kinovi sebagai `generation`, sehingga tarif per video lama **tidak** berubah menjadi per detik. Satuan baru hanya berlaku saat **Terapkan harga** menulis tarif dan satuannya sekaligus; model yang dikunci atau modalnya belum diketahui tetap per generation. Pratinjau menampilkan satuan saat ini dan satuan baru (catatan "Satuan tagihan berubah saat diterapkan"). Modal tersimpan dengan satuan berbeda dari target tampil sebagai galat baris dan dihitung `unit_mismatch` saat apply; masukkan ulang modal manual pada satuan target. Mengosongkan tarif lewat editor model juga mengosongkan satuannya.
- Modal fal per gambar/megapiksel dikalikan jumlah hasil maksimum yang dapat dipilih bila kontrak v2 menagih per permintaan (mis. `num_images` 1–4 → modal ×4); eksekusi native v1 menagih per hasil lewat jumlah dan tidak dikalikan lagi. Selama model masih berjalan native, batas harga (jumlah hasil, megapiksel, durasi) hanya memakai konfigurasi native dan revisi v2 yang sudah terbit; kandidat impor atau revisi yang baru ditinjau tidak memengaruhi harga (contoh: FLUX dev native dihitung dari ukuran terbesar 1792×1024 → 2 MP, bukan batas skema kandidat 200 MP). Jumlah maksimum yang tidak terbatas membuat modal tidak diketahui.
...
- Chat: buka riwayat lama, kirim/stop respons, pindah percakapan dengan draft/lampiran, simpan notes, lalu simpan dan unduh revisi artefak. Periksa chip saldo/harga model, draft tetap utuh pada 402, debit final/estimated/release, replay tanpa debit kedua, dan admin tanpa debit.
