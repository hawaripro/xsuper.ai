...
<main id="main-content" class="policy-page">
    <div class="container">
        <header class="policy-hero"><a class="text-link" href="{{ $localeUrl('/') }}">@include('public.partials.icon', ['name' => 'arrow', 'class' => 'back-arrow']) {{ __('Kembali ke UltrAI') }}</a><h1>{{ $policy['title'] }}</h1><p>{{ $policy['description'] }}</p></header>
        <div class="policy-layout">
            ...
            <div class="policy-content">
                ...
                @endforeach
                <section class="policy-contact"><h2>{{ __('Masih ada pertanyaan?') }}</h2><p>{{ __('Tim UltrAI dapat membantu menjelaskan kebijakan dan menangani permintaanmu.') }}</p><a class="button button-primary" href="https://wa.me/{{ $site['support']['phone'] }}">{{ __('Hubungi melalui WhatsApp') }} @include('public.partials.icon', ['name' => 'external'])</a></section>
            </div>
        </div>
    </div>
</main>
