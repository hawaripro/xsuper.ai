    ...
    <section class="model-marketplace" aria-labelledby="model-catalog-title">
        <div class="model-marketplace-shell">
            <header class="model-marketplace-header">
                <div><h1 id="model-catalog-title">{{ __('Katalog Model AI') }}</h1><p><output data-model-count>{{ count($models) }}</output> {{ __('model tersedia') }}</p><p>{{ __('Chat memakai PAYG (Saldo AI) per 1 juta token. Studio memakai token media sesuai model dan durasi.') }}</p></div>
                        ...
                        <div class="model-table-body" data-model-grid>
                            @foreach($models as $model)
                                ...
                                <article id="{{ $model['id'] }}" class="model-row" data-model-card data-model-search-text="{{ $search }}" data-model-provider="{{ strtolower($model['provider']) }}" data-model-capabilities="{{ strtolower(implode(' ', $model['capabilities'])) }}" data-model-modalities="{{ strtolower(implode(' ', $model['modalities'])) }}" data-model-billing="{{ $billing }}" data-model-context="{{ $model['context'] ?? 0 }}">
                                    <div class="model-name-cell"><img class="model-row-logo" data-logo-id="{{ $model['id'] }}" src="{{ $model['logo'] }}" width="32" height="32" alt="" loading="lazy" onerror="this.onerror=null;this.src='/xsuper-icon-v2.png'"><div><strong>{{ $model['name'] }}</strong><code>{{ $model['id'] }}</code><p>{{ $model['description'] }}</p><div class="model-row-badges">@foreach($model['badges'] as $badge)<span>{{ $badge }}</span>@endforeach @foreach($model['modalities'] as $modality)<span>{{ ucfirst($modality) }}</span>@endforeach</div></div><button type="button" class="model-copy" data-copy-model="{{ $model['id'] }}" aria-label="{{ __('Salin ID model') }}">@include('public.partials.icon', ['name' => 'copy'])</button></div>
                                    <span class="model-price-cell">{{ $billing === 'tokens' ? '—' : ($model['inputRate'] ? ($locale === 'en' ? '$'.number_format((float)$model['inputRate']['price_usd'], 3, '.', ',') : 'Rp '.number_format((float)$model['inputRate']['price_idr'], 0, ',', '.')) : __('Tarif belum tersedia')) }}</span>
                                    <span class="model-price-cell">
                                        @if($billing === 'tokens')
                                            ...
                                            @endif
                                        @else
                                            {{ $model['outputRate'] ? ($locale === 'en' ? '$'.number_format((float)$model['outputRate']['price_usd'], 3, '.', ',') : 'Rp '.number_format((float)$model['outputRate']['price_idr'], 0, ',', '.')) : __('Tarif belum tersedia') }}
                                        @endif
                                    </span>
