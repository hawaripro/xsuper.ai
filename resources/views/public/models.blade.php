                        ...
                        <div class="model-table-body" data-model-grid>
                            @foreach($models as $model)
                                ...
                                <article id="{{ $model['id'] }}" class="model-row" data-model-card data-model-search-text="{{ $search }}" data-model-provider="{{ strtolower($model['provider']) }}" data-model-capabilities="{{ strtolower(implode(' ', $model['capabilities'])) }}" data-model-modalities="{{ strtolower(implode(' ', $model['modalities'])) }}" data-model-billing="{{ $billing }}" data-model-context="{{ $model['context'] ?? 0 }}">
                                    <div class="model-name-cell"><img class="model-row-logo" data-logo-id="{{ $model['id'] }}" src="{{ $model['logo'] }}" width="32" height="32" alt="" loading="lazy" onerror="this.onerror=null;this.src='/brands/ultrai/mark-96.webp'"><div><strong>{{ $model['provider'] }}: {{ $model['name'] }}</strong><code>{{ $model['id'] }}</code><p>{{ $model['description'] }}</p><div class="model-row-badges">@foreach($model['badges'] as $badge)<span>{{ $badge }}</span>@endforeach @foreach($model['modalities'] as $modality)<span>{{ ucfirst($modality) }}</span>@endforeach</div></div><button type="button" class="model-copy" data-copy-model="{{ $model['id'] }}" aria-label="{{ __('Salin ID model') }}">@include('public.partials.icon', ['name' => 'copy'])</button></div>
