...
export default function MediaApiGuide({ baseUrl }) {
    ...
    return <section className="space-y-6" aria-labelledby="media-api-guide-title">
        ...
        <aside className="space-y-3 rounded-xl border border-slate-200 p-4 text-sm leading-6 text-slate-600 dark:border-white/10 dark:text-slate-300">
            ...
            <p>{t('OpenAI Images menunggu hingga 120 detik secara bawaan. Respons 504 menyertakan id pekerjaan dan URL polling; pekerjaan tetap berjalan. Gunakan URL tersebut dengan kunci Anda, jangan membuat ulang dengan kunci idempotensi baru. Respons 502 menjelaskan kegagalan yang aman dibaca anggota.')}</p>
            <p>{t('count, pro, billing_seconds, dan rights_confirmed tersedia pada endpoint media sesuai billing dan skema model. Avatar memerlukan konfirmasi hak atas foto dan suara. Untuk batch native, respons menyertakan generation_ids: pantau setiap id untuk mengambil semua hasil; total_tokens adalah total cadangannya.')}</p>
        </aside>
    </section>;
}
