...
export default function ModelBulkTable({ models, providers = [], onRefresh, onEdit, onToggle, mediaOnly = false, disabled = false }) {
    ...
    const tableInput = "ui-input min-h-10 min-w-28";
        ...
        {selected.length > 0 && <div className="flex flex-wrap items-end gap-3 bg-slate-50 p-4 dark:bg-white/5">
            <label className="text-xs font-medium">{t("Ubah pilihan sekaligus")}
                <select className="ui-input mt-1 min-h-10" value={massField} disabled={locked} onChange={(event) => { setMassField(event.target.value); setMassValue(event.target.value === "is_enabled" ? "false" : ""); }}>
                    <option value="token_cost">{t("Token per hasil")}</option><option value="category">{t("Kategori")}</option><option value="tier">{t("Tier")}</option><option value="is_enabled">{t("Publikasi")}</option><option value="sort_order">{t("Urutan tampil")}</option>
                </select>
            </label>
                ...
                <tbody>{pageRows.map((model) => {
                    ...
                    return <Fragment key={model.id}>
                        <tr className={`border-t border-slate-200 align-top dark:border-white/10 ${selection.has(model.id) ? "bg-red-50/60 dark:bg-red-500/5" : ""}`}>
                            ...
                            <td className="p-3"><input className={tableInput} aria-label={`${t("Tier")} ${model.model_id}`} value={row.tier || ""} maxLength={40} disabled={locked} onChange={(event) => patch(model.id, "tier", event.target.value)} />{fieldError(model.id, "tier")}</td>
