...
export default function AudioPlayer({ job }) {
    const { t } = useLocale();
    ...
    useEffect(() => {
        ...
        if (element) {
            element.volume = 0.8;
            if (element.getAttribute("src") !== job.audio_url) element.setAttribute("src", job.audio_url);
        }
        const decode = async () => {
            try {
                const AudioContextClass = window.AudioContext || window.webkitAudioContext;
                if (!AudioContextClass) throw new Error("Audio decoding unavailable");
                const response = await fetch(job.audio_url, { credentials: "same-origin", signal: controller.signal, headers: { Accept: "audio/*" } });
    ...
    return <div className="studio-audio-player">
        ...
        <div className="studio-toolbar studio-player-links"><p className="studio-help">{t(waveError ? "Waveform tidak tersedia untuk format ini." : "Waveform menampilkan amplitudo audio asli, bukan animasi simulasi.")}</p><a className="studio-button studio-download" href={job.audio_url} download><StudioIcon name="download" />{t("Unduh audio")}</a></div>
    </div>;
}
