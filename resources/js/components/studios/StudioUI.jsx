...
export function StudioCatalog({ studio, id, value, onChange, models = studio.models, error }) {
    ...
    return <><StudioField id={id} label="Model" error={error}><select id={id} value={chosen ? value : ""} onChange={(event) => onChange(event.target.value)} disabled={studio.submitting || studio.modelLoading || !models.length} aria-invalid={Boolean(error)}><option value="" disabled>{t(studio.modelLoading ? "Memuat…" : "Pilih model")}</option>{models.map((model) => <option key={model.id} value={model.id}>{model.name || model.id}</option>)}</select>{chosen && <p className="studio-help">{[chosen.provider, chosen.tier].filter(Boolean).join(" · ")}</p>}</StudioField>{studio.modelError && <StudioNotice error action={<StudioButton onClick={studio.loadModels} disabled={studio.modelLoading}>{t("Coba lagi")}</StudioButton>}>{t(mediaError(studio.modelError))}</StudioNotice>}{!studio.modelLoading && !studio.modelError && !models.length && <StudioNotice>{t("Tidak ada model tersedia")}. {t("Pengelola belum mengaktifkan model yang tersedia dan memiliki harga token untuk alur ini.")}</StudioNotice>}{studio.requestedModel && !chosen && !studio.modelLoading && models.length > 0 && <StudioNotice>{t("Model dari tautan tidak tersedia. Pilih model lain untuk melanjutkan.")}</StudioNotice>}</>;
}
export function StudioHistory({ studio, kind, title }) {
    const { t, locale } = useLocale();
    return <section className="studio-history" aria-label={t(title)}><div className="studio-section-heading"><h2><StudioIcon name="history" />{t(title)}</h2><StudioButton onClick={studio.loadHistory} disabled={studio.historyLoading} icon="refresh">{t("Perbarui riwayat")}</StudioButton></div>{studio.historyError && <StudioNotice error>{t("Riwayat belum dapat diperbarui. Periksa kembali sebelum mengirim permintaan baru.")}</StudioNotice>}{studio.historyLoading && !studio.jobs.length ? <p className="studio-loading" role="status">{t("Memuat riwayat…")}</p> : !studio.jobs.length ? <StudioEmpty icon="history" title="Belum ada riwayat" description="Permintaan dan hasil pada akun Anda akan muncul di sini." /> : <div className="studio-history-list">{studio.jobs.map((job) => <button className="studio-history-item" type="button" key={job.job_id} aria-pressed={studio.activeId === job.job_id} onClick={() => studio.selectJob(job)}><span className={`studio-history-thumb studio-color-${kind}`}>{kind === "image" && job.result_urls?.[0] ? <img src={job.result_urls[0]} alt="" loading="lazy" /> : kind === "video" && job.thumbnail_url ? <img src={job.thumbnail_url} alt="" loading="lazy" /> : <StudioIcon name={kind === "audio" ? job.mode === "speech" ? "voice" : "music" : kind} />}</span><span className="studio-history-copy"><strong>{job.prompt || t("Prompt tidak tersedia")}</strong><small>{job.model} · {formatLocalDate(job.created_at, { locale })}{job.pro_mode ? " · Pro" : ""}</small></span><StudioStatus job={job} /></button>)}</div>}</section>;
}
