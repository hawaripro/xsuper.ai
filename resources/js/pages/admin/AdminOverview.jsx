        ...
        <DashboardWorkspace title={t('Operasional XSuper.ai')} description={t('Pantau keuangan, pemakaian, dan pekerjaan operasional dalam satu ruang.')} actions={<><button type="button" className="dw-button" onClick={loadAll} disabled={loadingAny}>{Icons.refresh}<span>{t('Refresh data')}</span></button><Link className="dw-button dw-button-primary" to={localizedPath('/admin/token-usage')}>{Icons.token}{t('Tinjau pemakaian')}</Link></>}>
            ...
            <div className="dw-board">
                <div className="dw-stack">
                    <WorkspaceUsageChart rows={timeline} loading={sources.usage.loading} error={sources.usage.error ? t(sources.usage.error) : null} onRetry={() => loadSource('usage', usagePeriod)} description={`${t(usagePeriod === 'daily' ? 'Permintaan seluruh akun selama 30 hari terakhir.' : 'Permintaan seluruh akun selama 365 hari terakhir.')} ${timezone}`} reportPath="/admin/token-usage" controls={<div className="dw-periods" role="group" aria-label={t('Periode pemakaian')}><button type="button" aria-pressed={usagePeriod === 'daily'} onClick={() => setUsagePeriod('daily')}>{t('30 hari')}</button><button type="button" aria-pressed={usagePeriod === 'monthly'} onClick={() => setUsagePeriod('monthly')}>{t('1 tahun')}</button></div>} />
