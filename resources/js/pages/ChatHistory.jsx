            ...
            <button type="button" onClick={onOpen} className="min-w-0 flex-1 text-left focus:outline-none">
                <div className="flex items-start justify-between gap-2">
                    <h3 className="truncate text-[13px] font-semibold text-slate-900 dark:text-white">
                        {conversation.title || t("Percakapan tanpa judul")}
                    </h3>
                    {active && <StatusBadge value="active" label={t("Dibuka")} />}
                </div>
                <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                    {Number(conversation.message_count || 0).toLocaleString("id-ID")} {t("pesan")} · {formatLocalDate(conversation.last_message)}
                </p>
            </button>
            <button
                type="button"
                onClick={onDelete}
                disabled={deleting}
                aria-label={`${t("Hapus")} ${conversation.title || t("percakapan")}`}
    ...
    if (!conversation) {
        return (
            <div className="flex min-h-72 items-center justify-center p-5">
                <StatePanel
                    title={t("Pilih percakapan")}
                    description={t("Buka salah satu riwayat untuk membaca pesan lengkapnya.")}
                    compact
                />
            </div>
        );
    ...
    return (
        <div className="flex min-h-0 flex-1 flex-col">
            ...
            <div className="max-h-[620px] flex-1 space-y-3 overflow-y-auto p-4">
                {loading ? (
                    <div className="flex min-h-48 items-center justify-center"><Spinner label={t("Memuat pesan")} /></div>
                ) : error ? (
                    <StatePanel
                        type="error"
                        title={t("Percakapan tidak dapat dibuka")}
                        description={errorMessage(error, "Pesan percakapan gagal dimuat.")}
                        action={<Button variant="secondary" onClick={onRetry}>{t("Coba lagi")}</Button>}
                    />
                ) : messages.length === 0 ? (
                    <StatePanel title={t("Percakapan kosong")} description={t("Tidak ada pesan yang tersimpan pada percakapan ini.")} />
                ) : (
                            ...
                            <article key={`${message.created_at || "message"}-${index}`} className={`flex ${member ? "justify-end" : "justify-start"}`}>
                                <div className={`max-w-[88%] rounded-xl px-3 py-2.5 ${member ? "bg-red-600 text-white dark:bg-red-500" : "border border-slate-200 bg-slate-50 text-slate-800 dark:border-white/[0.08] dark:bg-white/[0.04] dark:text-slate-200"}`}>
                                    <div className="mb-1 flex flex-wrap items-center gap-2 text-[10px] font-semibold uppercase tracking-wide opacity-70">
                                        <span>{member ? t("Anda") : t("Asisten")}</span>
...
export default function ChatHistory() {
    ...
    const selectedId = requestedConversation && requestedConversation.length <= 100 ? requestedConversation : null;
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                const data = await apiRequest(`/api/c/h/${encodeURIComponent(selectedId)}`, { signal: controller.signal });
                if (!controller.signal.aborted) setMessages(Array.isArray(data?.messages) ? data.messages : []);
            } catch (error) {
    ...
    const deleteConversation = async (conversation) => {
        if (!window.confirm(`Hapus riwayat “${conversation.title || t("Percakapan tanpa judul")}”? Tindakan ini tidak dapat dibatalkan.`)) return;
        ...
        try {
            ...
            setSearchParams((current) => {
                if (current.get("conversation") !== conversation.conversation_id) return current;
                const next = new URLSearchParams(current);
    ...
    // Search can select an owned conversation older than the recent-history window.
    const selected = conversations.find((conversation) => conversation.conversation_id === selectedId)
        || (selectedId ? {
            conversation_id: selectedId,
            title: messages.find((message) => message.role === "user")?.content?.slice(0, 50),
    ...
    return (
        <MemberPage>
            <PageHeader
                eyebrow={t("Workspace")}
                title={t("Riwayat chat")}
                description={t("Cari, buka, dan hapus percakapan yang tersimpan pada akun Anda.")}
                actions={<Button variant="secondary" onClick={loadConversations} disabled={loading}>{loading ? t("Memuat…") : t("Muat ulang")}</Button>}
            />
            ...
            {actionError && (
                <StatePanel
                    type="error"
                    compact
                    title={t("Percakapan belum terhapus")}
                    description={errorMessage(actionError)}
                    action={<Button variant="ghost" onClick={() => setActionError(null)}>{t("Tutup")}</Button>}
                />
            )}
                ...
                <div className="grid min-h-[520px] lg:grid-cols-[340px_minmax(0,1fr)]">
                    <section className="border-b border-slate-200 lg:border-b-0 lg:border-r dark:border-white/[0.08]">
                        <div className="space-y-3 border-b border-slate-200 p-4 dark:border-white/[0.08]">
                            <SectionHeader title={t("Percakapan")} description={`${conversations.length.toLocaleString("id-ID")} ${t("riwayat tersimpan")}`} />
                            <label className="relative block">
                                <span className="sr-only">{t("Cari percakapan")}</span>
                                ...
                                <input className={`${controlClass} pl-9`} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("Cari judul chat…")} type="search" />
                            </label>
                        </div>
                        <div className="max-h-[620px] overflow-y-auto">
                            {loading ? (
                                <div className="flex min-h-48 items-center justify-center"><Spinner label={t("Memuat riwayat")} /></div>
                            ) : listError ? (
                                <div className="p-4"><StatePanel type="error" title={t("Riwayat tidak dapat dimuat")} description={errorMessage(listError)} action={<Button variant="secondary" onClick={loadConversations}>{t("Coba lagi")}</Button>} /></div>
                            ) : conversations.length === 0 ? (
                                <div className="p-4"><StatePanel title={t("Belum ada riwayat")} description={t("Percakapan baru akan tampil di sini setelah Anda menggunakan Chat AI.")} /></div>
                            ) : filtered.length === 0 ? (
                                <div className="p-4"><StatePanel title={t("Tidak ada hasil")} description={`${t("Tidak ada judul yang cocok dengan")} “${query.trim()}”.`} compact /></div>
                            ) : (
