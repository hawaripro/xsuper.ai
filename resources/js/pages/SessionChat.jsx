...
export default function SessionChat() {
    ...
    const isOnline = (lastActive) => {
        ...
        return (now - lastActive) < 300; // 5 minutes
    };
    ...
    const formatDate = (timestamp) => {
        if (!timestamp) return '—';
        return new Date(timestamp * 1000).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' });
    };
                ...
                {loading ? (
                    ...
                    <div className={`divide-y ${isDark ? 'divide-white/[0.04]' : 'divide-gray-100'}`}>
                        {nonAdminUsers.map((u, i) => (
                            <div
                                key={u.id}
                                className={`group px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-colors ${isDark ? 'hover:bg-white/[0.03]' : 'hover:bg-red-50/40'}`}
                            ...
                            >
                                <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="relative flex-shrink-0">
                                        ...
                                        <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 ${isDark ? 'border-gray-900' : 'border-white'} ${isOnline(u.last_active_at) ? 'bg-emerald-500' : 'bg-gray-400'}`} />
                                    </div>
                                    {/* Info */}
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-2">
                                            <span className={`text-sm font-semibold truncate ${head}`}>{u.name || '—'}</span>
                                            {/* Role badge */}
                                            <span className={`shrink-0 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider ${
                                                u.role === 'admin'
                                                    ? (isDark ? 'bg-red-500/15 text-red-300 border border-red-500/25' : 'bg-red-50 text-red-600 border border-red-200')
                                                    : (isDark ? 'bg-blue-500/15 text-blue-300 border border-blue-500/25' : 'bg-blue-50 text-blue-600 border border-blue-200')
                                            }`}>
                                                {u.role || 'user'}
                                            </span>
                                            ...
                                            <span className={`shrink-0 inline-flex items-center gap-1 text-[10px] font-semibold ${
                                                ...
                                                <span className={`w-1.5 h-1.5 rounded-full ${isOnline(u.last_active_at) ? 'bg-emerald-500 animate-pulse' : 'bg-gray-400'}`} />
                                                {isOnline(u.last_active_at) ? 'Online' : 'Offline'}
                                            </span>
                                        </div>
                                        <div className={`text-xs truncate mt-0.5 ${muted}`}>{u.email || '—'}</div>
                                        {/* Meta row */}
                                        <div className={`flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5 text-[10px] ${muted}`}>
                                            <span className="inline-flex items-center gap-1">
                                                <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                                                {formatTime(u.last_active_at)}
                                            </span>
                                            {u.timezone && (
                                                <span className="inline-flex items-center gap-1">
                                                    <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>
                                                    {u.timezone}
                                                </span>
                                            )}
                                            <span className="inline-flex items-center gap-1">
                                                <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
