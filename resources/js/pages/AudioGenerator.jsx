...
const defaults = { mode: "speech", speechModel: "", musicModel: "", speechPrompt: "", musicPrompt: "", voice: "", speed: "1", duration: "", tempo: "" };
...
function AudioStudio({ userId }) {
    ...
    const generate = (event) => {
        ...
        studio.submit({ model: model.id, mode, prompt: prompt.trim(), ...(mode === "speech" ? { voice: draft.voice, ...(speedAvailable ? { speed: Number(draft.speed) } : {}) } : { duration: Number(draft.duration), ...(draft.tempo ? { tempo: Number(draft.tempo) } : {}) }) });
    };
    return <div className="media-studio studio-audio">
        ...
        <section className="studio-audio-workbench">
            ...
            <form onSubmit={generate} aria-busy={studio.submitting} id={`audio-panel-${mode}`} role="tabpanel" aria-labelledby={`audio-tab-${mode}`}>
                <div className="studio-audio-editor-grid"><div className="studio-audio-editor"><div className="studio-section-heading"><h2>{t(mode === "speech" ? "Naskah voiceover" : "Arahan musik")}</h2><span className="studio-help">{prompt.length}/{limit}</span></div><StudioField id="audio-prompt" label={mode === "speech" ? "Teks yang akan diucapkan" : "Deskripsi komposisi"} error={errors.prompt} className="studio-audio-prompt"><textarea id="audio-prompt" rows={7} value={prompt} required maxLength={limit} disabled={studio.submitting} aria-invalid={Boolean(errors.prompt)} placeholder={t(mode === "speech" ? "Tulis naskah yang ingin dibacakan…" : "Jelaskan genre, instrumen, suasana, dan perkembangan musik…")} onChange={(event) => set(`${mode}Prompt`, event.target.value)} /></StudioField><div className="studio-audio-editor-note"><StudioIcon name={mode === "speech" ? "voice" : "music"} /><p>{t(mode === "speech" ? "Suara yang dihasilkan adalah suara AI, bukan rekaman manusia." : "Satu hasil audio utuh. Tidak ada stem, notasi, atau track terpisah yang dibuat oleh editor ini.")}</p></div>{mode === "speech" && englishVoices && <StudioNotice>{t("Pilihan suara model ini menggunakan bahasa Inggris (American English). Pelafalan bahasa lain tidak dijamin.")}</StudioNotice>}{mode === "music" && <div className="studio-music-direction"><StudioField id="audio-tempo" label="Arahan tempo (opsional)" error={errors.tempo} hint={t("40–200 BPM sebagai arahan prompt, bukan janji tempo yang presisi.")}><div className="studio-input-unit"><input id="audio-tempo" type="number" min="40" max="200" step="1" value={draft.tempo} disabled={studio.submitting} onChange={(event) => set("tempo", event.target.value)} aria-describedby="audio-tempo-hint" /><span>BPM</span></div></StudioField><p className="studio-help">{t("Arahan tempo ikut dihitung dalam batas karakter prompt oleh server.")}</p></div>}</div>
