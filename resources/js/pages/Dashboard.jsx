...
// Stat Card Component
// ============================================
function StatCard({ icon, label, value, suffix, trend, color = 'red' }) {
    const colors = {
    ...
    return (
        <div className={`relative p-5 rounded-2xl bg-gradient-to-br ${colors[color]} border backdrop-blur-xl overflow-hidden group hover:scale-[1.02] transition-transform duration-300`}>
...
// Quick Action Card
// ============================================
function QuickAction({ icon, title, desc, href, external = false }) {
    ...
    return (
        <Wrapper {...props} className="group flex items-center gap-4 p-4 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06] hover:border-white/[0.1] transition-all duration-200">
            ...
            <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold text-white group-hover:text-red-400 transition-colors">{title}</div>
                <div className="text-xs text-gray-500 truncate">{desc}</div>
            </div>
            <svg className="w-4 h-4 text-gray-600 group-hover:text-gray-400 group-hover:translate-x-0.5 transition-all flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round"><path d="M5 12h14" /><path d="m12 5 7 7-7 7" /></svg>
        </Wrapper>
    );
}
...
// ============================================
function ServiceStatus({ name, url, status = 'online' }) {
    const statusColors = {
    ...
    return (
        <div className="flex items-center justify-between py-3 border-b border-white/[0.04] last:border-0">
            ...
            <div className="flex items-center gap-3">
                ...
                <a href={url} target="_blank" rel="noopener noreferrer" className="text-xs text-gray-500 hover:text-red-400 transition-colors">
                    Buka →
                </a>
            </div>
        </div>
...
export default function Dashboard() {
    ...
    return (
        <div className="p-4 lg:p-6 space-y-6 max-w-7xl mx-auto">
            {/* Welcome Header */}
            <div className="relative p-6 lg:p-8 rounded-2xl bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 border border-white/[0.06] overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-red-500/[0.08] rounded-full blur-[80px] -translate-y-1/2 translate-x-1/4" />
                <div className="absolute bottom-0 left-1/2 w-48 h-48 bg-red-500/[0.04] rounded-full blur-[60px] translate-y-1/2" />
                ...
                <div className="relative z-10">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="px-3 py-1.5 rounded-lg bg-white/[0.06] border border-white/[0.08] text-xs font-mono text-gray-400">
            ...
            <div className="grid lg:grid-cols-3 gap-4 lg:gap-6">
                ...
                <div className="lg:col-span-2 space-y-4">
                    <div className="p-5 lg:p-6 rounded-2xl bg-gray-900/50 border border-white/[0.06] backdrop-blur-xl">
                    ...
                    <div className="p-5 lg:p-6 rounded-2xl bg-gray-900/50 border border-white/[0.06] backdrop-blur-xl">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {['GPT-4o', 'Claude Sonnet', 'Gemini Pro', 'DeepSeek V3', 'Llama 3.1', 'Mistral Large'].map((model) => (
                                <div key={model} className="px-3 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] text-sm text-gray-300 font-medium hover:bg-white/[0.06] hover:border-white/[0.1] transition-all cursor-default">
                                    <div className="flex items-center gap-2">
                ...
                <div className="space-y-4">
                    {/* Service Status — Live */}
                    <div className="p-5 rounded-2xl bg-gray-900/50 border border-white/[0.06] backdrop-blur-xl">
                        <h2 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
