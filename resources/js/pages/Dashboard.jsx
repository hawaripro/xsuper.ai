...
function PendingApproval({ orderId, isDark, onApproved }) {
    ...
    useEffect(() => {
        const poll = setInterval(async () => {
            try {
                const res = await fetch("/api/period/my-orders", {
                ...
                if (res.ok) {
                    ...
                    const order = data.orders?.find(
                        (o) =>
                            `ULTR-${String(o.id).padStart(4, "0")}` === orderId,
                    );
    ...
    return (
        <div className="text-center py-4">
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-amber-500/15 flex items-center justify-center">
                <svg
                    className={`w-8 h-8 animate-spin ${isDark ? "text-amber-400" : "text-amber-500"}`}
                    fill="none"
                    viewBox="0 0 24 24"
                >
                    <circle
            ...
            >
                Your order has been submitted. An admin will review it shortly.
            </p>
            <div
                className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-mono font-bold ${isDark ? "bg-white/[0.05] text-gray-300 border border-white/[0.08]" : "bg-gray-100 text-gray-600 border border-gray-200"}`}
            >
                Order: {orderId}-checking
            </div>
            <p
                className={`text-[10px] mt-3 ${isDark ? "text-gray-600" : "text-gray-400"}`}
            >
...
function StatCard({
    ...
    return (
        ...
        >
            {/* Accent glow */}
            <div
                className={`absolute -top-16 -right-16 w-40 h-40 rounded-full blur-3xl ${c.glow} opacity-60 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500`}
            />

            <div className="relative flex items-start justify-between mb-4">
                <div
                    className={`w-11 h-11 rounded-xl bg-gradient-to-br ${c.icon} text-white flex items-center justify-center shadow-lg shadow-black/10 ring-4 ${c.ring} group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}
                >
                    {icon}
                </div>
                {trend && (
            ...
            <div className="relative">
                <div
                    className={`text-3xl font-extrabold tracking-tight tabular-nums ${isDark ? "text-white" : "text-slate-900"}`}
                >
                    {animated ? <AnimatedNumber value={value} /> : value}
                    {suffix && (
                        <span className="text-lg opacity-70 ml-0.5">
                            {suffix}
                        </span>
                    )}
                </div>
                <div
                    className={`text-[13px] mt-1 font-medium ${isDark ? "text-gray-400" : "text-gray-500"}`}
                >
                    {label}
                </div>
            </div>
...
function QuickAction({
    ...
    return (
        ...
        >
            <div
                className={`relative w-11 h-11 rounded-xl bg-gradient-to-br ${c.icon} text-white flex items-center justify-center flex-shrink-0 shadow-md ring-4 ${c.ring} group-hover:scale-110 group-hover:rotate-3 transition-all duration-300`}
            >
                {icon}
            </div>
            <div className="flex-1 min-w-0">
                <div
                    className={`text-sm font-semibold group-hover:text-red-500 transition-colors ${isDark ? "text-white" : "text-slate-900"}`}
                >
                    {title}
                    {external && (
                        <svg
                            className="inline w-3 h-3 ml-1 opacity-50"
                            fill="none"
                            stroke="currentColor"
                            strokeWidth="2"
                            viewBox="0 0 24 24"
                ...
                <div
                    className={`text-xs truncate mt-0.5 ${isDark ? "text-gray-500" : "text-gray-500"}`}
                >
                    {desc}
                </div>
            </div>
...
function ServiceStatus({ name, url, status = "online", isDark }) {
    ...
    return (
        <div
            className={`flex items-center justify-between py-3 border-b last:border-0 transition-colors ${isDark ? "border-white/[0.05]" : "border-gray-100"}`}
        >
            <div className="flex items-center gap-3 min-w-0">
                <span className="relative flex h-2 w-2">
                    {status === "online" && (
                        <span
                            className="absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-70 animate-ping"
                            style={{ animationDuration: "2.2s" }}
                        />
                    )}
                    <span
                        className={`relative inline-flex w-2 h-2 rounded-full ${s.dot} shadow-[0_0_10px] ${s.glow}`}
                    />
                </span>
                <span
                    className={`text-sm font-medium truncate ${isDark ? "text-gray-200" : "text-slate-700"}`}
                >
                    {name}
                </span>
            </div>
...
export default function Dashboard() {
    ...
    const [durationPackages, setDurationPackages] = useState([]);
    ...
    useEffect(() => {
        fetch("/api/period/packages", {
            ...
            .then((response) =>
                response.ok
                    ? response.json()
                    : Promise.reject(new Error("Gagal memuat paket")),
            )
            .then((data) =>
                setDurationPackages(
    ...
    return (
        <div className="p-4 lg:p-6 space-y-5 lg:space-y-6 max-w-7xl mx-auto">
            ...
            >
                ...
                <div
                    className="absolute -bottom-32 left-1/3 w-96 h-96 rounded-full bg-orange-400/15 blur-[120px] animate-aurora"
                ...
                <div className="relative p-6 lg:p-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
                    <div className="flex-1 min-w-0">
                        ...
                        <h1
                            className={`text-2xl sm:text-3xl lg:text-4xl font-extrabold tracking-tight leading-[1.1] ${cardHead}`}
                        >
                            {greeting()},{" "}
                        ...
                        <p
                            className={`mt-2 text-sm sm:text-base max-w-xl ${muted}`}
                        >
                            Kelola semua layanan{" "}
                            <span className="font-semibold text-red-500">
                                UltrAI
                            </span>{" "}
                            dari sini. Lihat statistik, jalankan Chat AI, dan
                            pantau status layanan secara realtime.
                        </p>
                    ...
                    >
                        ...
                        <div
                            className={`text-4xl font-extrabold tabular-nums tracking-tight my-0.5 bg-gradient-to-br from-red-500 to-red-600 bg-clip-text text-transparent`}
                        >
                            {clock}
                        </div>
            ...
            {/* ============================================
               ...
               ============================================ */}
            <section
                className={`grid grid-cols-2 ${isAdmin ? "lg:grid-cols-4" : "lg:grid-cols-2"} gap-3 lg:gap-4 stagger`}
            >
                <StatCard
                    icon={Icon.chat}
                    label="Chat Sessions"
                ...
                {isAdmin && (
                    <>
                        <StatCard
                            ...
                            label="AI Models"
                            value={
                                aiStatus.chat_models ||
                                aiStatus.total_models ||
                                0
                            }
                            suffix="+"
                            color="blue"
            ...
            <section className="grid lg:grid-cols-3 gap-5 lg:gap-6">
                ...
                <div className="lg:col-span-2 space-y-5 lg:space-y-6">
                    {/* Quick actions */}
                    <div
                        className={`relative overflow-hidden p-5 lg:p-6 rounded-2xl border ${card} animate-fade-in-up`}
                    >
                        <div className="flex items-center justify-between mb-5">
                            <h2
                                className={`text-base lg:text-lg font-bold flex items-center gap-2 ${cardHead}`}
                            >
                        ...
                        <div className="grid sm:grid-cols-2 gap-3 stagger">
                            ...
                            >
                                ...
                                <div className="flex-1 min-w-0">
                                    <div
                                        className={`text-sm font-semibold group-hover:text-amber-500 transition-colors ${isDark ? "text-white" : "text-slate-900"}`}
                                    >
                                        Tambah Durasi
                                    </div>
                                    <div
                                ...
                                <svg
                                    className={`w-4 h-4 group-hover:translate-x-0.5 transition-all flex-shrink-0 ${isDark ? "text-gray-600 group-hover:text-gray-400" : "text-gray-300 group-hover:text-gray-500"}`}
                                    fill="none"
                                    stroke="currentColor"
                                    strokeWidth="2"
                                    viewBox="0 0 24 24"
                ...
                <div className="space-y-5 lg:space-y-6">
                    {/* Service status */}
                    <div
                        className={`p-5 rounded-2xl border ${card} animate-fade-in-up`}
                    >
                        <h2
                            className={`text-base lg:text-lg font-bold mb-4 flex items-center gap-2 ${cardHead}`}
                        >
                    ...
                    >
                        ...
                        <div className="space-y-3">
                            <div
                                className={`flex items-center justify-between p-3 rounded-xl ${isDark ? "bg-white/[0.03]" : "bg-gray-50/80"}`}
                            >
                                <span
                                ...
                                <span
                                    className={`text-sm font-semibold truncate ml-3 ${cardHead}`}
                                >
                                    {user?.name || "-"}
                                </span>
                            </div>
                            ...
                            {!isAdmin && (
                                ...
                                >
                                    ...
                                    {user?.is_expired ? (
                                        <span className="text-[11px] font-bold px-2 py-0.5 rounded-md bg-red-500/15 text-red-400 border border-red-500/20">
                                            Expired
                                        </span>
                                    ) : user?.days_remaining === null ? (
                                        <span className="text-[11px] font-bold px-2 py-0.5 rounded-md bg-emerald-500/15 text-emerald-400 border border-emerald-500/20">
                                            ∞ Unlimited
                                        </span>
            ...
            {devicePending && !deviceApproved && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                    ...
                    <div
                        className={`relative w-full max-w-sm rounded-2xl border p-6 text-center shadow-2xl ${isDark ? "bg-gray-900 border-white/[0.08]" : "bg-white border-gray-200"}`}
                    ...
                    >
                        ...
                        >
                            {devicePending.blocked ? (
                                ...
                                <>
                                    ...
                                    >
                                        {devicePending.device_name || "Unknown"}
                                    </span>{" "}
                        ...
                        {!devicePending.blocked && (
                            <div className="flex flex-col items-center gap-3">
                                <div className="flex items-center gap-2">
                                    <svg
                                        className={`w-5 h-5 animate-spin ${isDark ? "text-amber-400" : "text-amber-500"}`}
                                        fill="none"
                                        viewBox="0 0 24 24"
                                    >
                                        <circle
            ...
            {deviceApproved && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                    ...
                    >
                        ...
                        >
                            Akses telah diberikan. Halaman akan dimuat ulang...
                        </p>
                    </div>
                </div>
            )}
            ...
            {showDurasiModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    />
                    <div
                        className={`relative w-full max-w-md rounded-2xl border p-6 shadow-2xl ${isDark ? "bg-gray-900 border-white/[0.08]" : "bg-white border-gray-200"}`}
                        style={{ animation: "scale-in 0.2s ease-out" }}
                    >
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {durationPackages.map((pkg) => (
                                        ...
                                        >
                                            ...
                                            <div
                                                className={`w-9 h-9 rounded-xl bg-gradient-to-br ${pkg.popular ? "from-red-500 to-orange-500" : "from-slate-500 to-slate-600"} flex items-center justify-center text-white shadow-lg`}
                                            >
                                                <svg
                                                    className="w-4 h-4"
                                                    fill="none"
                                            ...
                                            <div
                                                className={`text-xs font-semibold ${pkg.popular ? "text-red-500" : isDark ? "text-gray-400" : "text-gray-500"}`}
                                            >
                                                Rp{" "}
                        ...
                        {orderStep === "qris" && selectedPkg && (
                            <div className="text-center">
                                ...
                                <div
                                    className={`inline-block p-4 rounded-2xl border mb-4 ${isDark ? "bg-white" : "bg-white border-gray-200 shadow-sm"}`}
                                >
                                    <img
                                        ...
                                        onError={(e) => {
                                            ...
                                            e.target.parentElement.innerHTML =
                                                '<div style="width:250px;height:250px;display:flex;align-items:center;justify-content:center;background:#f3f4f6;border-radius:12px;color:#6b7280;font-size:12px;">QRIS Image</div>';
                                        }}
                                    />
                                </div>
                                ...
                                <div className="flex items-center justify-center gap-2 mb-2">
                                    <svg
                                        className={`w-4 h-4 animate-spin ${isDark ? "text-amber-400" : "text-amber-500"}`}
                                        fill="none"
                                        viewBox="0 0 24 24"
                                    >
                                        <circle
                                        ...
                                        onClick={async () => {
                                            ...
                                            try {
                                                ...
                                                if (res.ok) {
                                                    setOrderId(
                                                        data.order?.id
                                                            ? `ULTR-${String(data.order.id).padStart(4, "0")}`
                                                            : "ULTR-0001",
                                                    );
                        ...
                        {orderStep === "pending" && (
                            <PendingApproval
                                orderId={orderId}
                                isDark={isDark}
                                onApproved={() => setOrderStep("success")}
                            />
                        )}
                        ...
                        {orderStep === "success" && (
                            <div className="text-center py-4">
                                ...
                                >
                                    Durasi akun Anda telah ditambahkan.
                                </p>
                                <button
                                    onClick={() => {
                                        setShowDurasiModal(false);
