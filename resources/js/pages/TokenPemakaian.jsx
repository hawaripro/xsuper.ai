...
function UsageOverview({ usage, wallet, tokens }) {
    ...
    return (
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <Metric label={t("Saldo wallet")} value={formatUsdMicros(wallet?.balance_microusd)} detail={t("Saldo penggunaan terukur")} />
            <Metric label={t("Saldo token")} value={formatCount(tokens?.balance)} detail={t("Token gambar & video")} />
            <Metric label={t("Permintaan")} value={formatCount(usage?.total_requests)} detail={t("Pada periode terpilih")} />
            <Metric label={t("Total token")} value={formatCount(usage?.total_tokens)} detail={`${formatCount(usage?.total_credits)} kredit`} />
        </div>
    );
}
...
export default function TokenPemakaian() {
    ...
    return (
        <MemberPage>
            <PageHeader
                eyebrow={t("Billing & usage")}
                title={t("Token & pemakaian")}
                description={t("Saldo nyata, penggunaan API, dan transaksi token pada akun Anda.")}
                actions={<Button variant="secondary" onClick={load} disabled={loading}>{loading ? t("Memuat…") : t("Muat ulang")}</Button>}
            />

            <Panel className="p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <SectionHeader title={t("Rentang pemakaian")} description={t("Ringkasan dan grafik mengikuti periode pilihan.")} />
                    <label className="w-full sm:w-44">
                        <span className="mb-1 block text-[11px] font-medium text-slate-500 dark:text-slate-400">{t("Periode")}</span>
                        <select className={controlClass} value={period} onChange={(event) => setPeriod(event.target.value)} disabled={loading}>
                            {PERIODS.map(([value, label]) => <option key={value} value={value}>{t(label)}</option>)}
                        </select>
                    </label>
                </div>
            </Panel>
            ...
            {loading ? (
                <Panel className="flex min-h-56 items-center justify-center"><Spinner label={t("Mengambil data pemakaian")} /></Panel>
            ) : error ? (
                <Panel className="p-4">
                    <StatePanel type="error" title={t("Data pemakaian tidak dapat dimuat")} description={errorMessage(error, "Saldo atau riwayat penggunaan gagal dimuat.")} action={<Button variant="secondary" onClick={load}>{t("Coba lagi")}</Button>} />
                </Panel>
            ) : (
                <>
                    ...
                    <div className="grid gap-5 xl:grid-cols-[minmax(0,1.45fr)_minmax(300px,.75fr)]">
                        <Panel className="p-4">
                            <SectionHeader title={t("Aktivitas permintaan")} description={data.usage?.from ? `Sejak ${formatLocalDate(data.usage.from)}` : t("Seluruh riwayat tersedia")} />
                            {Array.isArray(data.usage?.timeline) && data.usage.timeline.length > 0 ? (
                                <div className="mt-5 flex h-48 items-end gap-1 overflow-x-auto border-b border-slate-200 pb-1 dark:border-white/[0.08]" role="img" aria-label={t("Grafik jumlah permintaan per periode")}>
                                ...
                                <div className="mt-4"><StatePanel title={t("Belum ada pemakaian")} description={t("Aktivitas API pada periode ini akan tampil sebagai grafik.")} compact /></div>
                            )}
                            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                                <Metric label={t("Kredit")} value={formatCount(data.usage?.total_credits)} />
                                <Metric label={t("Biaya tercatat")} value={formatUsdMicros(data.usage?.total_cost_microusd)} />
                                <Metric label={t("Terakhir dipakai")} value={data.usage?.last_used_at ? formatLocalDate(data.usage.last_used_at, { year: undefined }) : t("Belum pernah")} />
                            </div>
                        </Panel>

                        <Panel className="p-4">
                            <SectionHeader title={t("Transaksi token")} description={t("Maksimal 50 transaksi terbaru.")} />
                            <div className="mt-3 max-h-72 divide-y divide-slate-100 overflow-y-auto dark:divide-white/[0.06]">
                                {data.transactions.length === 0 ? (
                                    <StatePanel title={t("Belum ada transaksi")} description={t("Top-up dan pemakaian token akan muncul di sini.")} compact />
                    ...
                    <Panel className="overflow-hidden">
                        <div className="flex flex-col gap-3 border-b border-slate-200 p-4 sm:flex-row sm:items-end sm:justify-between dark:border-white/[0.08]">
                            <SectionHeader title={t("Pemakaian per model")} description={t("Permintaan, token, kredit, dan biaya pada periode terpilih.")} />
                            <label className="w-full sm:w-48">
                                <span className="mb-1 block text-[11px] font-medium text-slate-500 dark:text-slate-400">{t("Filter model")}</span>
                                <input className={controlClass} value={service === "all" ? "" : service} onChange={(event) => setService(event.target.value || "all")} type="search" placeholder={t("Semua model")} />
                            </label>
                        </div>
                        {modelRows.length === 0 ? (
                            <div className="p-4"><StatePanel title={t("Tidak ada rincian model")} description={service === "all" ? t("Belum ada model yang digunakan pada periode ini.") : t("Tidak ada model yang cocok dengan filter.")} compact /></div>
                        ) : (
